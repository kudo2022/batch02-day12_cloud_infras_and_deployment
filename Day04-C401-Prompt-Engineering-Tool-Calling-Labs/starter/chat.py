...
def tool_results_message(events: list[dict[str, Any]]) -> dict[str, str]:
    return {
        ...
        "content": (
            ...
            "Chi dung cac ket qua tool nay. Neu nguoi dung muon digest va du lieu da san sang, "
            "hay goi render_digest. Neu khong, hay tra loi truc tiep bang tieng Viet va neu nguon khi co the."
        ),
    }
...
def assistant_tool_message(response_text: str | None, calls: list[ToolCall]) -> dict[str, str]:
    ...
    content = response_text or "Toi se goi cac tool phu hop."
    return {
        "role": "assistant",
...
def run_model_tool_loop(
    *,
    ...
    for round_index in range(1, max_tool_rounds + 1):
        ...
        for call in calls:
            console_print(f"TOOL {call.name}({json.dumps(call.args, ensure_ascii=False, sort_keys=True)})")
            ...
            if call.name == "ask_user":
                ...
                question = result.get("question") or call.args.get("question") or "Vui long cung cap thong tin con thieu."
                rounds.append(round_record)
                return {
    ...
    return {
        ...
        "assistant_text": f"Da dung sau {max_tool_rounds} vong goi tool. Hay kiem tra transcript de xem chi tiet.",
        "rounds": rounds,
        "tool_events": all_tool_events,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Interactive Military Tech Search Agent chat with transcript logging.")
    ...
    console_print(f"Military Tech Search Agent chat. artifact_version={artifact_version.artifact_version}")
    console_print("Type /exit to stop.")
    ...
    while True:
        ...
        console_print(f"Transcript saved: {transcript_path}")

    write_transcript(transcript_path, transcript)
    console_print(f"Final transcript: {transcript_path}")
